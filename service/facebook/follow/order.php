        ...
        <div class="card h-lg-100 overflow-hidden">
            ...
            <div class="card-body bg-light">
                ...
                <form action="" id="form_id" class="user" method="POST" accept-charset="utf-8">
                    <div class="mb-3">
                        <label>Nhập ID Profile hoặc Link Profile:</label>
                        ...
                        <label>(*) Nếu nhập <code>Link Profile</code> hệ thống sẽ tự động <code>Get ID Profile</code>
                        </label>
                    </div>
                    <div class="mb-3">
                        <label>Máy chủ:</label>
                        ...
                        <?php } ?>
                        <div class="alert alert-warning alert-dismissible fade show fs--1" role="alert"> - Sv1 số lượng tăng tối thiểu 100 và tối đa 50.000. người thật tự bấm tăng, nếu bạn muốn bảo hành có thể mua bảo hành trong 7 ngày. <br> - Sv2 số lượng tăng tối thiểu 100 và tối đa 50.000 lên nhanh sau khi order, tốc độ nhanh nhất của hệ thống <br> - Sv3 số lượng tăng tối thiểu 100 và tối đa 20.000. Nên tạo số lượng nhỏ để Test trước khi tăng. <br> - Sv4 số lượng tăng tối thiểu 1000 và tối đa 100.000 lên ngay khi order, không bảo hành <br> - Sv5 lên ngay khi order với tốc độ nhanh nhất hệ thống, không bảo hành <br> - Sv6 lên ngay khi order với tốc độ nhanh nhất hệ thống, không bảo hành <br> - Những máy chủ có bảo hành thì chỉ bảo hành khi tụt trên 70%. <br> - Nên buff dư cho khách tránh tụt nhé!. </div>
                        ...
                        <?php } ?>
                        <div class="alert alert-warning alert-dismissible fade show fs--1" role="alert"> - Sv1 số lượng tăng tối thiểu 100 và tối đa 50.000. người thật tự bấm tăng, nếu bạn muốn bảo hành có thể mua bảo hành trong 7 ngày. <br> - Sv2 số lượng tăng tối thiểu 100 và tối đa 50.000 lên nhanh sau khi order, tốc độ nhanh nhất của hệ thống <br> - Sv3 số lượng tăng tối thiểu 100 và tối đa 20.000. Nên tạo số lượng nhỏ để Test trước khi tăng. <br> - Sv4 số lượng tăng tối thiểu 1000 và tối đa 100.000 lên ngay khi order, không bảo hành <br> - Sv5 lên ngay khi order với tốc độ nhanh nhất hệ thống, không bảo hành <br> - Sv6 lên ngay khi order với tốc độ nhanh nhất hệ thống, không bảo hành <br> - Những máy chủ có bảo hành thì chỉ bảo hành khi tụt trên 70%. <br> - Nên buff dư cho khách tránh tụt nhé!. </div>
                        <?php if ($row['service_price_server_5'] != 0) { ?>
                        <div class="form-check">
                            <input class="form-check-input" id="buff_server_5" type="radio" onchange="bill();"
                                name="server" value="sv5" data-bs-toggle="collapse" />
                            <label class="form-check-label" for="buff_server_5">Server 6
                                (<b><?=$row['service_price_server_5'];?>₫</b>) <?=$row['service_info_server_5'];?> <b
                                    class="text-warning">(Hoạt động)</b></label>
                        </div>
                        <?php } ?>
                    </div>
                    <div class="mb-3">
                        ...
                        <div class="alert alert-warning alert-dismissible fade show fs--1 text-center" role="alert">
                            Tổng tiền = (Số lượng) x (Giá 1 theo dõi) </div>
                    </div>
                    <div class="mb-3">
                        <label>Ghi chú:</label>
...
function buy_service() {
    ...
    $.ajax({
        url: WEBSITE_URL + prefix + 'service/facebook/follow/order.php',
        type: 'POST',
        dataType: 'JSON',
        data: {
...
function bill() {
    ...
    if (server == 'sv1') {
        ...
        var price = <?=$row['service_price_server_5'];?>;
    }
